...
- 🏆 **Campaign Standings** — ratings, battles, wyrdstone, and gold, shown when the admin turns them on
...
The admin sees the same and also keeps the accounts and the campaign's switches: whether the **standings** are shown (hidden by default: no table on the front page, no rating on the cards; a warband's own page shows its keeper their numbers); a health check of database, secrets and sign-in; every player with their role, sessions and last visit; granting or taking the game master's role; signing a player out everywhere and issuing a reset word when a password is forgotten; burning a ledger to start it afresh or releasing a warband from its keeper; running midnight by hand and reading the history of runs.
