...
Players sign in with an email and a password (no emails are sent; a recovery phrase stands in for a reset link) and take up one warband each. The ledger is kept on the server, so it follows the player from phone to laptop, and the Town Cryer can print what happened to every warband. Nights turn at midnight in the campaign's time zone (`timezone` in `campaign.json`). A night's result is still a deterministic function of the orders given: the server runs the same pure engine the browser used to.
...
4. **Sign-in is email and password**, handled by Better Auth. No emails are sent: the address is only the name a player signs in with, so there is no verification and no reset link. Instead every player gets a **recovery phrase** when they sign up (six words and a tail, shown once, stored hashed). "Forgotten your password?" at the Ledger trades the phrase for a new password, signs every device out, and issues a fresh phrase. A player who has lost the phrase as well gets a new one from the game master in the Watch House. Redeploy after setting the variables so the functions start with them.
