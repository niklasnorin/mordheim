...
- The campaign is managed on the site. Each player keeps one warband and tends its roster and story; a game master sets up the scenarios ahead of a game (when, in both calendars; who; which rulebook scenario or what custom rules; a prologue), writes the battle afterwards and may open it to the players to retell (earlier tellings kept), and everyone who fought adds their own prologue and epilogue, who they brought and how each fared, and who put whom out of action. While the game is played, the battle tracker at `/scenarios/<id>/battle/` is the table's phone: the turn, the tally, and a shared log of notes, scores and takedowns by turn, which the scenario page reads back. Game masters also write the Town Cryer's articles and edit the Curfew's content documents; an admin grants the role and keeps the accounts. The repository holds the rules, the code and the seed for an empty database.
