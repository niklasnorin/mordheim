...
| **Resting, Recovering** | Resting: went out last night, stays home tonight. Recovering: injured in the most recent battle report, stays home until the player marks them fit. The dead stay home. | `restingMembers`, `recoveringMembers`, `heal`, `injuredInLastBattle` in `roster.ts` |
...
| **Curse** | Content only, in `tokens.json` `curses`. Never dropped by dice or absence; reserved for opt-in risk in the unbuilt mini-games. | none yet |
...
| **Debug strip** | Previous and next night buttons and a date picker in the Ledger's footer, on with `CURFEW_DEBUG`. Its per-device switch lives in `localStorage`. | `src/curfew/debug.ts`, `env.CURFEW_DEBUG` |
