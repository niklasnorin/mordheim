...
| **Crossroads** | About one night in three (`crossroadsChance` is the raw draw; the rest after brings it down), one member out on real orders meets one: the Dawn Report stops right after that member's paragraph and the player chooses a **road** there. Never on standing orders or twice running; unmet crossroads first, the least recently met again once all are met. Undecided at the next midnight, the character takes the `default` road. Content per place (`crossroads`, `undecided`, `markNames`); the night stores what was met and, once decided, the road, its words and its ledger line. See `src/data/curfew/CROSSROADS.md`. | `Crossroad`, `Road`, `CrossroadsMet`, `takeRoad`, `defaultRoad` in `engine.ts`; `decide`, `settleWaiting`, `waitingCrossroads` in `ledger.ts`; `decide` action |
...
| **Out of action** | A confirmed takedown, recorded by the one who struck or the one who fell, at the table (with its turn) or afterwards (without). | `OutOfAction`, `OutOfAction.turn`, `addOutOfAction`, `removeOutOfAction` |
| **Battle tracker, the table** | `/scenarios/<id>/battle/`: the game as it is played, on a phone. The turn, the tally, the log. "At the table" means the keepers of the attending warbands and the game masters, who alone write in it. | `src/lib/tracker.ts`, `mayTrack`, `setTurn` |
| **Calling the game, wrapping up** | Marking the scenario played with every warband's result, from the tracker. The table's to do; reopening is a game master's. | `markPlayed`, `reopenScenario` |
| **Turn** | A game turn as the table counts it, 1 upwards; 0 before the first. Kept on the scenario; every logged line carries one. | `Scenario.turn`, `ScenarioEvent.turn` |
| **Tally** | What a scenario scores per warband besides who fell: shards on a Wyrdstone Hunt, warriors through on a Breakthrough, or whatever the game master names. A skirmish tallies nothing. | `Scenario.tally`, `tallyOf`, `RULEBOOK_TALLIES`, `scoresOf` |
...
| **Place band** | The letterhead on both Curfew pages: a band of the place's own painting with its name over it, and a 96px mark beside the night line. Cut by `scripts/curfew/render-places.py` into `public/curfew/places/`, framed by the pack's `art.focus`. | `src/components/PlaceBand.astro`, `.cf-place*` in `curfew.css` |
| **The Odds** | The game master's page at `/admin/odds/`: outcome, charm, crossroads and Cryer odds by tilt, expected yield per errand, and a simulation of real nights. Reads the engine's constants; the dials are code. | `src/curfew/odds.ts`, `src/pages/admin/odds.astro` |
