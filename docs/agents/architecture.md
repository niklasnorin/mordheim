...
2. `todayFor(url)` in `src/server/curfew/service.ts` is the only place the server decides which night is current. It honours `?date=` only when `env.CURFEW_DEBUG` is on. The cron passes `null`, so it can never be steered. With the Debug strip on, a game master's requests are dry runs (`src/server/curfew/dry.ts`): `withLedger` computes from a sandbox the browser carries and neither saves nor publishes.
