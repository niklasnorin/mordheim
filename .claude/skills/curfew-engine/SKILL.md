...
4. **The location of a night is `locationForNight(night, moves)`**, never the current place. Every engine function that depends on a place takes `location: Location = DEFAULT_LOCATION`. The content itself (`OMENS`, `MOONS`, `TOKENS`, `LOCATIONS`…) are live bindings the server sets from the database with `useContent()` (`primeContent()` in `src/server/content/curfew.ts`, called at every service entry point); the files are the built-in defaults the tests and the browser start from, and the Curfew pages hand the browser the database's content in `boot.content`. The roster, likewise, is loaded from the database (`loadRoster()`) and passed to the pure helpers in `src/curfew/roster.ts`.
