...
1. **Call the game**, in the battle tracker (`/scenarios/<id>/battle/`, the button under the scenario's title; **The battle** drawer on the page points there too): every attending warband gets `victory`, `defeat` or `draw`, suggested from the tally where it decides. Anyone at the table may; the scenario joins the Chronicle in play order and its Imperial date follows from the real one. Only a game master reopens it.
