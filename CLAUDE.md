...
- `/admin/` is the Watch House, the game master's console: `src/pages/admin/index.astro`, `src/server/admin/service.ts` (overview, players, burn/release, notices, midnight by hand, moving the campaign), `src/pages/api/admin/[action].ts`. `/admin/odds/` (`src/pages/admin/odds.astro`, `src/curfew/odds.ts`) lays out the odds by tilt and simulates nights, read from the engine's constants (`outcomeOdds`, `TOKEN_CHANCE`, `FAVOUR`, `YIELDS`, `PRINT_CHANCE`, `crossroadsChance`); change a dial there, not on the page. Admission by `ADMIN_EMAILS` (`isAdminEmail` in `env.ts`); under the dev sign-in everyone is admitted. Styles in `src/styles/admin.css` on top of Curfew's tokens.
...
- Content is data: `src/data/warbands.ts`, `news.ts` (articles carry a `location`; the Cryer prints the ones for where the campaign is), `history/*.json`, `curfew/*.json`, `curfew/locations/*.json`. A place's band and mark (`public/curfew/places/`, cut by `scripts/curfew/render-places.py`, framed by `art.focus` in the pack) are content too: `src/components/PlaceBand.astro` is the letterhead both Curfew pages carry. Prose follows `src/data/curfew/STYLE.md`; the plan and status live in `src/data/curfew/PLAN.md`.
