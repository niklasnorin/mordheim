...
- The campaign's record is in the database and managed on the site: warbands and members by their owner or a game master, scenarios by game masters and the players who fought (with the battle tracker at `/scenarios/<id>/battle/` logging the game turn by turn as it is played), the Cryer's articles and the Curfew's content documents by game masters in the Watch House. The files under `src/data/` are the seed for an empty database and the engine's built-in fixture; editing them changes nothing on a seeded database. Rules, the campaign's constants and code stay source. Three roles: player, game master (`user.role`), admin (`ADMIN_EMAILS`).
